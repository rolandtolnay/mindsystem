...
Bridges the gap between "capture for later" (/gsd:add-todo) and "full phase workflow" (/gsd:insert-phase).
...
<execution_context>
@~/.claude/get-shit-done/workflows/do-work.md
@.planning/STATE.md
</execution_context>
...
Validate project has .planning/STATE.md (active GSD project required).
</step>

<step name="analyze_scope">
Quick analysis of what tasks are needed.
If >2 tasks or architectural changes required: REFUSE with suggestion to use /gsd:insert-phase.
</step>
...
If Rule 4 triggered (architectural): STOP, suggest /gsd:insert-phase.
</step>
...
<anti_patterns>
- Don't use for work requiring >2 tasks (use /gsd:insert-phase)
- Don't use for work requiring architectural changes
- Don't use outside active GSD projects (needs STATE.md)
