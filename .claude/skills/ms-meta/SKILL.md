...
**Instruction-following budget (~150-250).** LLMs follow ~150-250 behavioral instructions with reasonable consistency before sharp decline (ManyIFEval, alphaXiv). At 500 instructions, even frontier models hit ~68% compliance. Each "do X" / "do NOT X" competes with all others — compliance follows multiplicative decay (individual success rate ^ instruction count). Reference data and structural markers cause less interference because they're retrieved, not obeyed. Meta-commentary ("this is important because...") adds context load without producing any behavioral change. When designing commands, workflows, or agents, every behavioral instruction must earn its place by measurably changing output in the runtime context.
...
- **Script** (`scripts/*.sh`) — Deterministic automation. No judgment logic.
