...
**Clarity-driven stop conditions.** When commands involve user discovery (discuss-phase, new-milestone, add-phase, adhoc), define stop conditions by downstream artifact quality — not by round count. The test: "Can I confidently write the artifact this command produces (CONTEXT.md, PROJECT.md, phase specification)?" Session length is an emergent property of the conversation's complexity, not an input to constrain. Numeric estimates in prompts ("2-4 rounds typical") create premature finalization pressure. Replace with clarity checks that reference the specific downstream consumer.
...
| Plan inline metadata | verifier + executor |
